...
const mealLabels={before:'До еды',after:'После еды',any:'Неважно'};
const details=m=>[categoryLabel(m),m.dose,m.meal&&mealLabels[m.meal],m.note].filter(Boolean).map(esc).join(' · ');
...
function openEditor(id){
 ...
 f.elements.scheduleMode.value=m?.durationDays?'duration':m?.cycle?'cycle':'weekly';f.elements.durationDays.value=m?.durationDays||7;f.elements.cycleOn.value=m?.cycle?.on||1;f.elements.cycleOff.value=m?.cycle?.off||1;f.elements.category.value=m?.category||'pill';updateCategory();updateScheduleMode();$('#editor').showModal();
}
function updateCategory(){const f=$('#form');f.elements.name.placeholder=f.elements.category.value==='action'?'Например, измерить давление':'Название из назначения'}
...
function updateScheduleMode(){
 ...
 $('#end-field').hidden=mode==='duration';$('#form').elements.end.disabled=mode==='duration';$('#start-label').textContent=mode==='cycle'?'Первый день приёма':'Начало курса';
}
