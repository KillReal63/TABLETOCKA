...
render=function(){
 ...
 $('#focus-hero').innerHTML=focus?`<section class="md-focus-main"><div class="md-focus-time">${esc(focus.time)}</div><div class="md-focus-object"><span class="md-pill gold" aria-hidden="true"></span></div><h2>${esc(focus.med.name)}</h2><p>${esc(focus.med.dose)}${focus.med.note?' · '+esc(focus.med.note):''}</p></section><button class="md-primary" data-take="${esc(focus.key)}">✓ Я принял</button><div class="md-focus-secondary"><button class="md-text-button" data-edit="${esc(focus.med.id)}">О препарате</button><button class="md-text-button" data-view="history">Всё расписание</button></div>`:`<section class="md-focus-main"><div class="md-focus-time">${list.length?'✓':'—'}</div><div class="md-focus-object"><span class="md-pill gold" aria-hidden="true"></span></div><h2>${list.length?'Всё на сегодня':state.meds.length?'Сегодня без приёмов':'Пока нет приёмов'}</h2><p>${list.length?'Все приёмы отмечены.':state.meds.length?'По твоему расписанию сегодня нет приёмов.':'Добавь препарат и время из своего назначения.'}</p></section><button class="md-primary" ${state.meds.length?'data-view="history"':'data-add'}>${state.meds.length?'Посмотреть расписание':'＋ Добавить препарат'}</button>`;
