...
render=function(){
 ...
 $('#focus-hero').innerHTML=focus?`<section class="md-focus-main"><div class="md-focus-time">${esc(focus.time)}</div><div class="md-focus-object"><span class="md-pill gold" aria-hidden="true"></span></div><h2>${esc(focus.med.name)}</h2><p>${esc(focus.med.dose)}${focus.med.note?' · '+esc(focus.med.note):''}</p></section><button class="md-primary" data-take="${esc(focus.key)}">✓ Я принял</button><div class="md-focus-secondary"><button class="md-text-button" data-edit="${esc(focus.med.id)}">О препарате</button><button class="md-text-button" data-view="history">Всё расписание</button></div>`:`<section class="md-focus-main"><div class="md-focus-time">${list.length?'✓':'—'}</div><div class="md-focus-object"><span class="md-pill gold" aria-hidden="true"></span></div><h2>${list.length?'Всё на сегодня':'Пока нет приёмов'}</h2><p>${list.length?'Все приёмы отмечены.':'Добавь препарат и время из своего назначения.'}</p></section><button class="md-primary" ${list.length?'data-view="history"':'data-add'}>${list.length?'Посмотреть отметки':'＋ Добавить препарат'}</button>`;
 ...
 $('#summary').textContent=list.length?`Отмечено ${completed.length} из ${list.length}`:'Расписание появится здесь';
